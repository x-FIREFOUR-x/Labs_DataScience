...
SELECT IdCar, IdYear, IdBody, IdFuel, IdTransmission,
	   Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
FROM (	 
		SELECT	CarSales.Brand, CarSales.Model, CarSales.[Year], CarSales.Body, CarSales._Fuel, CarInform.TransmissionType, 
				Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
		FROM CarSales
...
SELECT IdCar, IdYear, IdBody, IdFuel, IdTransmission,
	   Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
FROM (	 
		SELECT	CarInform.Brand, CarInform.Model, CarInform.[Year], CarInform.Body, CarEmissions.Fuel, CarInform.TransmissionType, 
				Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
		FROM CarSales
		FULL JOIN CarInform ON CarSales.Brand = CarInform.Brand and CarSales.Model = CarInform.Model and CarSales.[Year] = CarInform.[Year]
		FULL JOIN CarEmissions ON CarSales.Brand = CarEmissions.Brand and CarSales.Model = CarEmissions.Model and CarSales.[Year] = CarEmissions.[Year]
...
SELECT IdCar, IdYear, NULL, IdFuel, NULL,
	   Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
FROM (	 
		SELECT	CarEmissions.Brand, CarEmissions.Model, CarEmissions.[Year], NULL AS Body, CarEmissions.Fuel, NULL AS Transmissions, 
				Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
		FROM CarSales
		FULL JOIN CarInform ON CarSales.Brand = CarInform.Brand and CarSales.Model = CarInform.Model and CarSales.[Year] = CarInform.[Year]
		FULL JOIN CarEmissions ON CarSales.Brand = CarEmissions.Brand and CarSales.Model = CarEmissions.Model and CarSales.[Year] = CarEmissions.[Year]
...
INSERT INTO	[FactTable]
SELECT IdCar, IdYear, IdBody, IdFuel, NULL AS Transmission,
	   Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
FROM (	 
		SELECT	CarSales.Brand, CarSales.Model, CarSales.[Year], CarSales.Body, CarSales._Fuel, CarInform.TransmissionType, 
				Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
...
INSERT INTO	[FactTable]
SELECT IdCar, IdYear, IdBody, NULL AS Fuel, IdTransmission,
	   Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
FROM (	 
		SELECT	CarInform.Brand, CarInform.Model, CarInform.[Year], CarInform.Body, NULL AS Fuel, CarInform.TransmissionType, 
				Price,  MSRP, EngineHP, EngineVolume , [CO2(g/km)], [EmissionsCO(mg/km)]  
		FROM CarSales
		FULL JOIN CarInform ON CarSales.Brand = CarInform.Brand and CarSales.Model = CarInform.Model and CarSales.[Year] = CarInform.[Year]
		FULL JOIN CarEmissions ON CarSales.Brand = CarEmissions.Brand and CarSales.Model = CarEmissions.Model and CarSales.[Year] = CarEmissions.[Year]
